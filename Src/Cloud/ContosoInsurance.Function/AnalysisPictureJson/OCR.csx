...
{
    public static async Task UpdateAsync<T>(T t, OcrResults partycard, OcrResults partylicense, OcrResults partyplate)
        where T : class
    {
        await Task.Run(() => UpdateProperties(t, ImageKind.InsuranceCard, partycard));
        await Task.Run(() => UpdateProperties(t, ImageKind.DriverLicense, partylicense));
        await Task.Run(() => UpdateProperties(t, ImageKind.LicensePlate, partyplate));
    }
